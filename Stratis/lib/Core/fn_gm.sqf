...
JK_fnc_gm = {
	...
	if (true) then {
	    player addAction ["End Mission",
	    	...
	    	true,
	    	"",
	    	"player == player && (name player in JK_var_Zeus_Actions_Player || player in JK_var_Zeus_Actions)&& !(GeCoendMission)"
	    ];
	    ...
	    player addAction ["Yes - End Mission",
         	{
         		["gecoEnd", "BIS_fnc_endMissionServer",false,false,false] call BIS_fnc_MP;
         	},
         	nil,
         	99,
         	false,
