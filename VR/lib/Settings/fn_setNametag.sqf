...
if (_mode in ["RED","YELLOW","GREEN","BLUE","MAIN"]) exitWith {
    if !(assignedTeam player isEqualTo _mode) then {
        ...
        hintSilent format[localize "STR_GECO_NAMETAGS_YOURENOW",localize (format["STR_GECO_NAMETAGS_%1",_mode])];
    } else {
        hintSilent format[localize "STR_GECO_NAMETAGS_YOUREDNOW",localize (format["STR_GECO_NAMETAGS_%1",_mode])];
    };
};
...
if (_mode isEqualTo "Leave") exitWith {
    ...
    hintSilent localize "STR_GECO_LeaveGroup";
};

if (_mode isEqualTo "Join") exitWith {
    JK_NameTags_var_JoinGroup = !JK_NameTags_var_JoinGroup;
    hintSilent format[localize "STR_GECO_GM_JoinGroup",[localize "STR_GECO_GM_Disabled",localize "STR_GECO_GM_Enabled"] select JK_NameTags_var_JoinGroup];
};

if (_mode isEqualTo "Leader") exitWith {
    group player selectLeader player;
    hintSilent localize "STR_GECO_TakeLeader";
};
