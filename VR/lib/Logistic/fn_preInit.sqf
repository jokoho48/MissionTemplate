...
[
    {format [localize "STR_GECO_JK_DRAG", getText(configFile >> "CfgVehicles" >> typeof cursorTarget >> "displayName")]},
    ...
    "(isNull assignedGunner _target) && !(_target in JK_var_ChooseLoadout_Boxes) && !(_target isKindOf 'Pod_Heli_Transport_04_base_F') && !(_target isKindOf 'Slingload_base_F') && isNull (player getVariable ['JK_DragDrop_Item', objNull]) && isNull (_target getVariable ['JK_DragDrop_Player', objNull])",
    {
        ...
        if (_currentWeight >= __MAXWEIGHT) exitWith {
            hint format[localize "STR_GECO_JK_TOHEAVY", _currentWeight - __MAXWEIGHT];
        };
...
[
    {format [localize "STR_GECO_JK_DRAG_MED", (if ((player getVariable ["JK_LastTarget", cursorTarget]) isKindOf "Man") then {name (player getVariable ["JK_LastTarget", cursorTarget])} else {getText(configFile >> "CfgVehicles" >> typeof (player getVariable ["JK_LastTarget", cursorTarget]) >> "displayName")})]},
    ["Man"],
    3,
